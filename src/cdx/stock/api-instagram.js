...
class Instagram {
  init(
    ...
    const _cookieStore = new FileCookieStore(`./cookies/${username}.json`);

    const jar = request.jar(_cookieStore)
  ...
  async binding() {
    const wrapper = (method) => (...options) => {
      const trying = async (stop) => {
        try {
          return await method(...options);
        } catch(error) {
          if (stop) return authErrorResponse;
  ...
  async getFullInfo(username) {
    return this.request(`${username || this.credentials.username}/?__a=1`)
      .then(data => data.graphql.user);
  }
...
class MongoWrapper extends Instagram {
...
module.exports = (config, db) => new MongoWrapper(config, db);
